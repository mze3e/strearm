...
FIREBASE_API_KEY = st.secrets['firebase_credentials']['api_key']
...
def authenticate_user(
    ...
    url = f"{POST_REQUEST_URL_BASE}signInWithPassword?key={FIREBASE_API_KEY}"
    payload = {
        "email": email,
        "password": password,
...
def forgot_password_form(preauthorized: Union[str, Sequence[str], None]) -> None:
    ...
    url = f"{POST_REQUEST_URL_BASE}sendOobCode?key={FIREBASE_API_KEY}"
